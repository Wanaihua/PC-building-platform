    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="headerBg" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="200" align="center" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="handleOrder(scope.row.id)">下单<i class="el-icon-edit"></i> </el-button>
...
export default {
  ...
  methods: {
    ...
    query(){
      this.request.get("/shopcar/myShopCar",{
        params:{
          userId:this.userId,
        }}).then(res=>{
    ...
    handleOrder(id){
      ...
      if(address=="地址"||phone=="电话"){
        this.$message.error('请先到右上角完善个人信息');
        return;
      }
      this.$confirm('此操作将下单该套配置, 是否继续?', '提示', {
    ...
    handleDelete(id){
      this.$confirm('此操作将永久删除该配置, 是否继续?', '提示', {
        ...
        this.request.delete("/shopcar/"+id).then(res=>{
          if(res.code==='200'){
            this.$message({
              message: '删除成功',
              type: 'success'
    ...
    handleEdit(row){
      this.ModelId = row.id;
      this.request.get("/shopcar/"+this.ModelId).then(res=>{
