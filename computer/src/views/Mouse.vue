<template>
  <div>
    ...
    <div style="padding: 10px 0">
      ...
      <el-upload
          action="http://localhost:8090/mouse/import" :show-file-list="false" accept="xlsx" :on-success="handleExcelImportSuccess" style="display: inline-block"> <!--上传地址-->
    ...
    <el-dialog title="鼠标信息" :visible.sync="dialogFormVisible" width="30%">
      <el-form label-width="80px">
        <el-upload
          ...
          <img v-if="form.mouseImg" :src="form.mouseImg" class="avatar">
        ...
        <el-form-item label="鼠标">
          <el-input v-model="form.mouseName" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="品牌">
          <el-input v-model="form.mouseFactory" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="价格">
          <el-input v-model="form.mousePrice" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="类型">
          <el-input v-model="form.mouseType" autocomplete="off"></el-input>
        </el-form-item>
        <el-form-item label="创建时间" style="display: none">
          <el-input v-model="form.mouseTime" autocomplete="off"></el-input>
        </el-form-item>
      </el-form>
...
export default {
  ...
  methods:{
    ...
    save(){
      ...
      this.form.mouseTime = this.nowTime;
      this.dialogFormVisible = false;
      this.request.post("/mouse",this.form).then(res=>{
        if(res.code==='200'){
          this.$message({
            message: '保存成功',
            type: 'success'
    ...
    handleDelete(id){
      this.$confirm('此操作将永久删除该商品, 是否继续?', '提示', {
        ...
        this.request.delete("/mouse/"+id).then(res=>{
          if(res.code==='200'){
            this.$message({
              message: '删除成功',
              type: 'success'
    ...
    deleteBatch() {
      ...
      this.$confirm('此操作将永久删除这些商品, 是否继续?', '提示', {
        ...
        console.log(ids);
        this.request.post("/mouse/deleteBatch", ids).then(res => {
          if (res.code==='200') {
            this.$message({
              message: '删除成功',
              type: 'success'
    ...
    exp(){
      this.$confirm('此操作将导出主板, 是否继续?', '提示', {
        ...
        window.open("http://localhost:8090/mouse/export");
        this.$message({
          message: '导出成功',
          type: 'success'
        });
