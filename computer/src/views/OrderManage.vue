<template>
  <div>
    ...
    <el-table :data="tableData" border stripe :header-cell-class-name="headerBg" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="操作" width="200" align="center" fixed="right">
        <template slot-scope="scope">
          ...
          <el-button type="text" size="small" @click="handleSend(scope.row.id)">发货<i class="el-icon-edit"></i> </el-button>
    ...
    <el-dialog title="物流派送" :visible.sync="dialogFormVisibleTwo" width="30%">
      <el-form label-width="80px" size="small">
        <el-form-item label="">
          <el-input v-model="form.sendName" placeholder="请输入快递公司"></el-input>
        </el-form-item>
        <el-form-item label="">
          <el-input v-model="form.sendNumber" placeholder="请输入快递单号"></el-input>
        </el-form-item>
        <el-form-item label="">
...
export default {
  ...
  methods: {
    ...
    handleDelete(id){
      this.$confirm('此操作将执行退款, 请确定用户是否需要退款,是否继续?', '提示', {
    ...
    handleSend(id){
      this.$confirm('此操作将执行发货, 请确定用户是否需要发货,是否继续?', '提示', {
        confirmButtonText: '确定',
        cancelButtonText: '取消',
        type: 'warning'
      }).then(() => {
    ...
    sendGoods(){
      this.request.put("/order/sendGoods",this.form).then(res=>{
        if(res.code==='200'){
          this.$message({
            message: '发货成功',
            type: 'success'
